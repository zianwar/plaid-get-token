/*
  ...
  https://github.com/plaid/quickstart/tree/master/node

  Usage:

  $ node server.js <account_name> <output_token_path>
*/
...
let tokenDest = process.argv[3]
if (!tokenDest) {
  throw new Error('Invalid token destination path "' + tokenDest + '"')
}
...
function saveAccessToken(token) {
  ...
  fs.writeFileSync(tokenDest, token)
  console.log('Saved.')
}
...
var server = app.listen(APP_PORT, function() {
  ...
  console.log(`\nToken will be saved to ${tokenDest}`);
});
